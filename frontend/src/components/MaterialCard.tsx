...
import { 
  Card, Typography, IconButton, Box, Stack 
} from '@mui/material';
import { 
  Favorite, FavoriteBorder, ThumbUp, ThumbUpOutlined, 
  ThumbDown, ThumbDownAltOutlined, Delete 
...
const MaterialCard = ({ material, onDelete }: MaterialCardProps) => {
  ...
  const likesCount = baseLikes + (vote === 'like' ? 1 : 0) - (vote == null ? 0 : 0);
  const dislikesCount = baseDislikes + (vote === 'dislike' ? 1 : 0) - (vote == null ? 0 : 0);
  ...
  return (
    <Card sx={{ 
      ...
      <Typography variant="caption" sx={{ fontWeight: 'bold', color: 'text.secondary', ml: 1, mb: 1 }}>
      ...
      <Box sx={{ display: 'flex', gap: 2 }}>
        ...
        >
          <img src="https://via.placeholder.com/130x90?text=PDF" alt="preview" style={{ borderRadius: '4px' }} />
        </Box>
        ...
        <Box sx={{ flexGrow: 1 }}>
          <Typography 
            variant="h6" 
            onClick={() => navigate(`/material/${material.id}`)}
            sx={{ cursor: 'pointer', fontWeight: 'bold', lineHeight: 1.2, mb: 0.5 }}
          >
            {material.title}
          </Typography>
          ...
          <Stack spacing={0.2} sx={{ mt: 1 }}>
            <Typography variant="caption" color="text.secondary">
              <strong>Descripción:</strong> {material.description || 'Sin descripción adicional'}
            </Typography>
            <Typography variant="caption" color="text.secondary">
        ...
        <Stack direction="row" spacing={2} sx={{ alignItems: 'center' }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <IconButton size="small" onClick={handleToggleLike}>
              {vote === 'like' ? <ThumbUp fontSize="small" color="primary" /> : <ThumbUpOutlined fontSize="small" />}
            </IconButton>
           ...
           <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <IconButton size="small" onClick={handleToggleDislike}>
              {vote === 'dislike' ? <ThumbDown fontSize="small" color="error" /> : <ThumbDownAltOutlined fontSize="small" />}
            </IconButton>
